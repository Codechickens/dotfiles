...
Rectangle {
    ...
    Rectangle {
        ...
        width: {
            ...
            // Add extra padding to prevent text cutoff (8px extra for safety)
    ...
    Loader {
        ...
        sourceComponent: Component {
            PanelWindow {
                ...
                screen: {
                    // Find the screen where the dropdown should appear
                    if (typeof Quickshell !== "undefined" && Quickshell.screens && Quickshell.screens.length > 0) {
                        // Try to find the screen containing the dropdown
                        const dropdownPos = root.mapToGlobal(0, 0)
                        for (let i = 0; i < Quickshell.screens.length; i++) {
                            const s = Quickshell.screens[i]
                            if (dropdownPos.x >= s.x && dropdownPos.x < s.x + s.width &&
                                dropdownPos.y >= s.y && dropdownPos.y < s.y + s.height) {
                                return s
                            }
                        }
                ...
                onShowPopupChanged: {
                    if (showPopup) {
                        ...
                        // Position the popup with better edge handling
                        updatePosition()
                    } else {
                ...
                // Handle actual visibility after animation
                Timer {
                    id: hideTimer
                ...
                function updatePosition() {
                    const dropdownPos = dropdown.mapToGlobal(0, dropdown.height + 4)
                    const screen = dropdownMenu.screen

                    if (!screen) {
                        popupBackground.x = dropdownPos.x - (root.popupWidthOffset / 2)
                        popupBackground.y = dropdownPos.y
                        return
                    }

                    // Calculate position with bounds checking
                    let targetX = dropdownPos.x - (root.popupWidthOffset / 2)
                    let targetY = dropdownPos.y

                    // Ensure popup stays within screen bounds horizontally
                    const minX = 10
                    const maxX = screen.width - popupBackground.width - 10
                    targetX = Math.max(minX, Math.min(maxX, targetX))

                    // Ensure popup stays within screen bounds vertically
                    const maxY = screen.height - popupBackground.height - 10
                    targetY = Math.min(maxY, targetY)

                    popupBackground.x = targetX
                    popupBackground.y = targetY
                }
                ...
                MouseArea {
                    ...
                    onClicked: {
                        // Only close if clicking outside the popup content
