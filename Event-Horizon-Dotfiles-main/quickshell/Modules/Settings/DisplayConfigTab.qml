...
Item {
    ...
    function checkEdidForMonitor(monitorName, index) {
        // Try multiple methods to detect HDR: ddcutil first (more reliable), then EDID parsing
        edidCheckProcess.command = ["sh", "-c", "monitor=\"" + monitorName + "\"; " +
            // Method 1: Check Hyprland colorManagementPreset for HDR indicators
            "hyprctl monitors all 2>/dev/null | grep -A 50 \"$monitor\" | grep -q 'colorManagementPreset.*hdr' && echo 'HDR' && exit 0; " +
            // Method 2: Use ddcutil to check for HDR capabilities
            "ddcutil detect --terse 2>/dev/null | grep -i \"$monitor\" >/dev/null && " +
            "ddcutil capabilities 2>/dev/null | grep -qiE '(hdr|bt\\.2020|rec\\.2020|wide.*color.*gamut|color.*space.*extended)' && echo 'HDR' && exit 0; " +
            // Method 3: EDID parsing with improved regex
            "for card in /sys/class/drm/card*\"$monitor\"/edid; do " +
            "if [ -r \"$card\" ] 2>/dev/null; then " +
            "output=$(cat \"$card\" 2>/dev/null | edid-decode 2>&1); " +
            "echo \"$output\" | grep -qiE '(hdr.*static.*metadata|hdr.*metadata.*block|hdr.*static|hdr.*support|bt\\.2020|rec\\.2020|dci.*p3|wide.*color.*gamut|extended.*color.*space)' && echo 'HDR' && exit 0; " +
            "fi; done"]
