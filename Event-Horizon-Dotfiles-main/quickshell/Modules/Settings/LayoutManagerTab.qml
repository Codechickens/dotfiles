...
Item {
    ...
    function loadSavedLayouts() {
        ...
        try {
            ...
            // Try to find all .json files in the DarkMaterialShell layouts directory
            try {
                if (typeof Quickshell !== "undefined" && typeof Quickshell.exec === "function") {
                    // Use ls to list all files in the directory, then filter for .json
                    var listResult = Quickshell.exec(["ls", "-1", darkMaterialDir])
                    console.log("Directory listing result:", listResult)
                    if (listResult && listResult.trim()) {
                        var files = listResult.trim().split('\n')
                        console.log("All files in directory:", files)
                        files.forEach(function(file) {
                            if (file.trim() && file.endsWith('.json')) {
                                var fullPath = darkMaterialDir + file.trim()
                                console.log("Adding JSON file:", fullPath)
                    ...
                    console.log("Quickshell.exec not available")
                }
            } catch (e) {
                console.log("Could not list files in DarkMaterialShell directory:", e)
            }

            // If Quickshell.exec is not available or no files found, try known files
            if (possibleFiles.length === 0) {
                console.log("No layout files found via directory listing, trying known files...")

                // Try some known layout files that might exist
                var knownFiles = ["topbar_right___dock.json"]
                knownFiles.forEach(function(fileName) {
                    var filePath = darkMaterialDir + fileName
                    // Check if file exists by trying to read it
                    try {
                        var testFile = Qt.createQmlObject('
                            import QtQuick
                            import Quickshell.Io
                            FileView {
                                blockLoading: true
            ...
            possibleFiles.forEach(function(filePath) {
                try {
                    console.log("Trying to load:", filePath)
                    // Create FileView to read each layout file
                    ...
                    if (layoutFile) {
                        ...
                        console.log("FileView created for:", filePath)
                        var fileContent = layoutFile.text()
                        console.log("File content length:", fileContent ? fileContent.length : "null/undefined")
                        console.log("File content preview:", fileContent ? fileContent.substring(0, 100) : "empty")
                        if (fileContent && fileContent.trim()) {
                            console.log("Successfully loaded layout from:", filePath)
                            try {
                                var layout = JSON.parse(fileContent.trim())
                    ...
                    console.log("Failed to load layout from", filePath, ":", e.message)
                }
            })
        } catch (e) {
    ...
    ColumnLayout {
            ...
            Flow {
                ...
                Repeater {
                    ...
                    StyledRect {
                        ...
                        radius: Theme.cornerRadius
                        color: layoutMouseArea.containsMouse ? Qt.rgba(Theme.primary.r, Theme.primary.g, Theme.primary.b, 0.08) : Theme.surfaceContainer
                        border.color: layoutMouseArea.containsMouse ? Theme.primary : Qt.rgba(Theme.outline.r, Theme.outline.g, Theme.outline.b, 0.2)
                        border.width: 1
                        ...
                        Column {
                            ...
                            StyledRect {
                                ...
                                height: 32
                                radius: Theme.cornerRadius
                                color: layoutMouseArea.containsMouse ? Theme.primary : Qt.rgba(Theme.primary.r, Theme.primary.g, Theme.primary.b, 0.1)
                                border.color: Theme.primary
                                border.width: 1
                                ...
                                Row {
                                    ...
                                    EHIcon {
                                        name: "check_circle"
                                        size: 14
                                        color: layoutMouseArea.containsMouse ? Theme.primaryText : Theme.primary
                                        anchors.verticalCenter: parent.verticalCenter
                                    }
        ...
        ColumnLayout {
            ...
            // Refresh button (only show if there are layouts)
            StyledRect {
                ...
                visible: savedLayoutsModel.count > 0
                    ...
                    Repeater {
                        ...
                        StyledRect {
                            ...
                            radius: Theme.cornerRadius
                            color: layoutMouseArea.containsMouse ? Qt.rgba(Theme.secondary.r, Theme.secondary.g, Theme.secondary.b, 0.08) : Theme.surfaceContainer
                            border.color: layoutMouseArea.containsMouse ? Theme.secondary : Qt.rgba(Theme.outline.r, Theme.outline.g, Theme.outline.b, 0.2)
                            border.width: 1
                            ...
                            Column {
                                ...
                                Row {
                                    ...
                                    StyledText {
                                        ...
                                        color: Theme.surfaceText
                                        anchors.verticalCenter: parent.verticalCenter
                                        width: parent.width - Theme.iconSize - Theme.spacingS - 60
                                        elide: Text.ElideRight
                                    }
                                ...
                                StyledRect {
                                    ...
                                    height: 36
                                    radius: Theme.cornerRadius
                                    color: layoutMouseArea.containsMouse ? Qt.rgba(Theme.secondary.r, Theme.secondary.g, Theme.secondary.b, 0.8) : Theme.secondary
                                    border.color: Theme.secondary
                                    border.width: 1
