...
Item {
    ...
    PanelWindow {
        ...
        function updatePosition() {
            ...
            // Simple positioning based on triggerSection
            if (section === "bottom") {
                // Bottom bar -> Centered horizontally, above the trigger
                // We use triggerX + triggerWidth/2 as center
                const centerX = appDrawerPopout.triggerX + appDrawerPopout.triggerWidth / 2
                targetX = centerX - popupW / 2
                targetY = appDrawerPopout.triggerY - popupH - spacing
            } else if (section === "top") {
                 // Top bar -> Centered horizontally, below the trigger
                const centerX = appDrawerPopout.triggerX + appDrawerPopout.triggerWidth / 2
                targetX = centerX - popupW / 2
                targetY = appDrawerPopout.triggerY + 48 + spacing // Assuming bar height or triggerY is top
            } else if (section === "left") {
                // Left bar -> Centered vertically, right of trigger
                targetX = appDrawerPopout.triggerX + appDrawerPopout.triggerWidth + spacing
                targetY = (appDrawerPopout.triggerY) - popupH / 2 + 24 
            ...
            // Constrain to screen
            // Since PanelWindow is full screen relative to the screen output (usually), 
            // 0,0 is top-left of screen.
            
            // Wait, screen.x/y are global coordinates. 
            // PanelWindow with anchors.fill acts as an overlay on that screen.
            // Coordinate system inside PanelWindow: (0,0) is top-left of THAT screen.
            // But triggerX/triggerY are usually global coordinates?
            // If triggerX includes screen.x, we need to subtract screen.x.
            
            let localTargetX = targetX
            let localTargetY = targetY
            
            // Check if triggerX is global
            if (appDrawerPopout.triggerX >= screenX) {
                 localTargetX -= screenX
            }
            if (appDrawerPopout.triggerY >= screenY) {
