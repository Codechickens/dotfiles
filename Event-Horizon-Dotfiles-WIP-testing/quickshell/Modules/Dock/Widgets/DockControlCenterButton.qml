...
Rectangle {
    ...
    MouseArea {
        ...
        cursorShape: Qt.PointingHandCursor
        onClicked: {
            // Left click: toggle the control center
            if (popupTarget) {
                popupTarget.toggle()
            }
            root.clicked()
        ...
        onPressAndHold: {
            // Right click (press and hold): show context menu
            if (popupTarget && popupTarget.showContextMenu) {
                const rect = parent.mapToItem(null, 0, 0, width, height)
                popupTarget.showContextMenu(rect.x + rect.width / 2, rect.y + rect.height / 2, parentScreen)
            }
        }
    }
