...
Item {
    ...
    property real popupWidth: 850 * (SettingsData.appDrawerScale || 1.0)
    property real popupHeight: 800 * (SettingsData.appDrawerScale || 1.0)
    ...
    PanelWindow {
        ...
        color: "transparent"
            ...
            function updatePosition() {
                if (!appDrawerPopout.triggerScreen) {
                    console.log("[AppDrawerPopout] No trigger screen, cannot update position")
                    return
                }
                ...
                const barHeight = (appDrawerPopout.barThickness || 48)
                ...
                if (section === "topbar") {
                    section = appDrawerPopout.barPosition
                }
                
                console.log(`[AppDrawerPopout] Updating position. Trigger: (${appDrawerPopout.triggerX}, ${appDrawerPopout.triggerY}), Section: ${section}, BarHeight: ${barHeight}`)
        
                // Positioning logic adapted from TopBarControlCenterPopout
                if (section === "top") {
                    // Menu positioned below top bar, anchored to button's horizontal position
                    // Center horizontally on triggerX
                    targetX = appDrawerPopout.triggerX - popupW / 2
                    targetY = screenY + barHeight + appDrawerPopout.popupDistance + 15
                } else if (section === "bottom") {
                    // Menu positioned above bottom bar, anchored to button's horizontal position
                    targetX = appDrawerPopout.triggerX - popupW / 2
                    targetY = screenY + screen.height - barHeight - appDrawerPopout.popupDistance - popupH - 15
                } else if (section === "left") {
                    // Menu positioned to the right of left bar, anchored to button's vertical position
                    targetX = screenX + barHeight + appDrawerPopout.popupDistance + 15
                    targetY = appDrawerPopout.triggerY - popupH / 2
                } else if (section === "right") {
                    // Menu positioned to the left of right bar, anchored to button's vertical position
                    targetX = screenX + screen.width - barHeight - appDrawerPopout.popupDistance - popupW - 15
                    targetY = appDrawerPopout.triggerY - popupH / 2
                } else {
                    // Fallback center screen
                    targetX = (screen.width - popupW) / 2
                    targetY = (screen.height - popupH) / 2
                }
                
                console.log(`[AppDrawerPopout] Calculated target: (${targetX}, ${targetY})`)
                
                // Adjust for screen offset if needed (PanelWindow usually relative to screen)
                // If triggerX is absolute (includes screenX), and PanelWindow is relative to screen, we need to subtract screenX
                // But TopBarControlCenterPopout seems to use screen coordinates directly?
                // Let's assume PanelWindow is on the correct screen, so 0,0 is screen top-left.
                
                let localTargetX = targetX
                let localTargetY = targetY
                
                if (targetX >= screenX && screenX > 0) {
                     localTargetX -= screenX
                }
                if (targetY >= screenY && screenY > 0) {
                     localTargetY -= screenY
                }
        
                // Clamp to screen bounds
                localTargetX = Math.max(spacing, Math.min(localTargetX, screen.width - popupW - spacing))
                localTargetY = Math.max(spacing, Math.min(localTargetY, screen.height - popupH - spacing))
        
                console.log(`[AppDrawerPopout] Final position: (${localTargetX}, ${localTargetY})`)
        ...
        Rectangle {
            ...
            // AppDrawer background with custom transparency
            color: Qt.rgba(0.1, 0.1, 0.1, Math.max(0.5, SettingsData.appDrawerTransparency || 0.92))
            ...
            // Context Menu inside launcherPanel to keep relative positioning simple
            Rectangle {
                id: contextMenu
                // ... same implementation ...
            ...
            // MouseAreas for closing context menus when clicking elsewhere in the launcherPanel
            MouseArea {
                anchors.fill: parent
