        ...
        onPressed: {
            if (popupTarget && popupTarget.setTriggerPosition) {
                ...
                if (root.isBarVertical) {
                    ...
                    barPos = root.barPosition; // "left" or "right"
                    barHeight = SettingsData.topBarHeight || 32;
                    triggerY = rect.y + rect.height / 2;
                } else {
                    // Horizontal bar (top or bottom)
                    barPos = root.barPosition; // Use the property set by parent
                    barHeight = barPos === "bottom" ? (SettingsData?.taskBarHeight || 48) : (SettingsData.topBarHeight || 32);
                    triggerY = barPos === "bottom" ? (currentScreen.height - barHeight) : 0;
                }
                ...
                console.log(`[ControlCenterButton] Calling setTriggerPosition: (${triggerX}, ${triggerY}), width=${rect.width}, section="right"`);
                popupTarget.setTriggerPosition(triggerX, triggerY, rect.width, "right", currentScreen);
            }
            root.clicked();
        }
    }
