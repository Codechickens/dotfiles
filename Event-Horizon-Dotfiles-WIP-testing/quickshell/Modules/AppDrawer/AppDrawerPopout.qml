...
EHPopout {
    ...
    autoFitWidth: {
        const value = SettingsData?.topBarAutoFit || false;
        console.log(`[AppDrawerPopout] autoFitWidth binding evaluated: SettingsData.topBarAutoFit=${SettingsData?.topBarAutoFit}, result=${value}`);
        return value;
    }
    
    onAutoFitWidthChanged: {
        console.log(`[AppDrawerPopout] autoFitWidth changed to: ${autoFitWidth}`);
    }
    
    // Update bar position when settings change
    ...
    function setTriggerPosition(x, y, width, section, screen) {
        console.log(`[EHPopout] ${objectName} setTriggerPosition: (${x}, ${y}), width=${width}, section="${section}", screen=${screen ? screen.width + 'x' + screen.height : 'null'}`);
    ...
    popupWidth: 850 * (SettingsData.appDrawerScale || 1.0)
    popupHeight: 800 * (SettingsData.appDrawerScale || 1.0)
