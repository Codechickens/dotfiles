...
StyledRect {
    ...
    Column {
        ...
        Item {
            ...
            Row {
                ...
                StyledRect {
                    id: centerButton
                    height: 32
                    width: centerButtonText.implicitWidth + Theme.spacingM * 2
                    radius: Theme.cornerRadius
                    ...
                    StateLayer {
                        ...
                        onClicked: {
                            arrangementFlickable.centerContent()
                        }
                    }
                }
        ...
        Flickable {
            ...
            // Center the content when bounds change
            function centerContent() {
                if (bounds.width === 0 || bounds.height === 0) return
                
                // Calculate the center of the monitor arrangement
                var arrangementCenterX = (bounds.width * scaleFactor) / 2
                var arrangementCenterY = (bounds.height * scaleFactor) / 2
                
                // Calculate the center of the visible area
                var visibleCenterX = width / 2
                var visibleCenterY = height / 2
                
                // Position content so arrangement is centered
                // Add 20px offset for the margin used in monitor positioning
                contentX = Math.max(0, arrangementCenterX - visibleCenterX + 20)
                contentY = Math.max(0, arrangementCenterY - visibleCenterY + 20)
            }
            ...
            Item {
                ...
                Repeater {
                    ...
                    function checkCollision(newX, newY) {
                        ...
                        // Use a small padding to allow slight overlap during drag
                        var padding = 2
                        var newRight = newX + scaledWidth - padding
                        var newBottom = newY + scaledHeight - padding
                        var newLeft = newX + padding
                        var newTop = newY + padding
                        ...
                        for (var i = 0; i < root.monitors.length; i++) {
                            ...
                            var otherScaledW = otherWidth * scale
                            var otherScaledH = otherHeight * scale
                            var otherRight = otherScaledX + otherScaledW - padding
                            var otherBottom = otherScaledY + otherScaledH - padding
                            var otherLeft = otherScaledX + padding
                            var otherTop = otherScaledY + padding
                            ...
                            if (newLeft < otherRight &&
                                ...
                                newTop < otherBottom &&
                                newBottom > otherTop) {
                                // Only consider it a collision if there's meaningful overlap
                                var overlapX = Math.min(newRight, otherRight) - Math.max(newLeft, otherLeft)
                                var overlapY = Math.min(newBottom, otherBottom) - Math.max(newTop, otherTop)
                                if (overlapX > 0 && overlapY > 0) {
                                    // Require at least 5% overlap in both dimensions to avoid false positives
                                    var minOverlapX = Math.min(scaledWidth, otherScaledW) * 0.05
                                    var minOverlapY = Math.min(scaledHeight, otherScaledH) * 0.05
                    ...
                    MouseArea {
                        ...
                        onPressed: mouse => {
                            ...
                            dragStartX = monitorDelegate.x
                            dragStartY = monitorDelegate.y
                            isDragging = false
                            hasMoved = false
                        }
                        ...
                        onPositionChanged: mouse => {
                            if (drag.active) {
                                if (!hasMoved) {
                                    // Allow initial movement even if there's a collision
                                    hasMoved = true
                                }
                                isDragging = true
                                ...
                                // Don't snap during drag - it makes dragging feel stuck
                                // Snapping will happen on release instead

                                // Only check collision if we've moved significantly from start
                                // This prevents getting stuck when starting from a collision state
                                var movedDistance = Math.sqrt(Math.pow(newX - dragStartX, 2) + Math.pow(newY - dragStartY, 2))
                                
                                // Allow dragging to continue even if there's a collision
                                // We'll resolve collisions on release
                                // Only prevent movement if we're trying to move into a significant collision
                                // and we've moved a reasonable distance
                                if (movedDistance > 20 && checkCollision(newX, newY)) {
                                    // Try to find a nearby valid position, but don't block dragging
                                    var foundValid = false
                                    var offsets = [0, 5, 10, 15, 20, -5, -10, -15, -20]
                                    for (var i = 0; i < offsets.length && !foundValid; i++) {
                                        for (var j = 0; j < offsets.length && !foundValid; j++) {
                                            var testX = newX + offsets[i]
                                            var testY = newY + offsets[j]
                                    ...
                                    // If no valid position found nearby, allow the drag to continue anyway
                                    // The collision will be resolved on release
                                }
                        ...
                        onReleased: {
                            ...
                            var monitorHeight = (caps.height || 1080) / parseFloat(monitor.scale || "1.0")
                            var monitorRight = actualX + monitorWidth
                            var monitorBottom = actualY + monitorHeight
                            var alignmentThreshold = 20 // pixels in actual coordinates

                            // Apply snapping on release for better alignment
                            var scaledX = monitorDelegate.x
                            var scaledY = monitorDelegate.y
                            var snappedScaledX = findSnapX(scaledX)
                            var snappedScaledY = findSnapY(scaledY)
                            
                            // Convert snapped scaled coordinates back to actual coordinates
                            var snappedActualX = ((snappedScaledX - 20) / arrangementArea.scaleFactor) + arrangementArea.bounds.minX
                            var snappedActualY = ((snappedScaledY - 20) / arrangementArea.scaleFactor) + arrangementArea.bounds.minY
                            
                            // Use snapped position if close, otherwise use calculated position
                            var snapThreshold = 30 / arrangementArea.scaleFactor
                            if (Math.abs(snappedActualX - actualX) < snapThreshold) {
                                actualX = snappedActualX
                            }
                            if (Math.abs(snappedActualY - actualY) < snapThreshold) {
                                actualY = snappedActualY
                            }
                            ...
                            for (var i = 0; i < root.monitors.length; i++) {
                                ...
                                var otherBottom = otherPos.y + otherHeight
                                ...
                                // Vertical alignment
                                var distanceToBottom = Math.abs(actualY - otherBottom)
                                if (distanceToBottom < alignmentThreshold) {
                                    actualY = otherBottom
                                    break
                                }
                                
                                var distanceToTop = Math.abs(monitorBottom - otherPos.y)
                                if (distanceToTop < alignmentThreshold) {
                                    actualY = otherPos.y - monitorHeight
                                    break
                                }
                            }
