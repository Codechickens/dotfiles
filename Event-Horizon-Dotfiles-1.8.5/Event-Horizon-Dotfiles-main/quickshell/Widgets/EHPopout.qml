...
PanelWindow {
    ...
    onPopupHeightChanged: if (shouldBeVisible) updatePosition()
    ...
    function updatePosition() {
        ...
        if (root.barPosition === "top") {
            ...
            if (root.objectName === "appDrawerPopout") {
                ...
                // Control Center: Position below the bar, but ensure it doesn't go off-screen
                targetX = Math.max(15, Math.min(screenWidth - root.popupWidth - 15, triggerX - root.popupWidth / 2));  // Center on trigger
                // Calculate base Y position (below bar)
                const baseY = screenY + scaledBarThickness + root.popupDistance + 10;
                // Ensure popup doesn't go off the bottom of the screen
                const maxY = screenY + screenHeight - root.popupHeight - 15;
                targetY = Math.min(maxY, baseY);
                console.log(`  Top bar ControlCenter: targetX=${targetX}, targetY=${targetY} (constrained to screen)`);
            }
            else {
            ...
            // ===== CONTROL CENTER POPUP =====
                ...
                // Calculate target Y based on section, ensuring it doesn't go off the top of the screen
                targetX = Math.max(15, Math.min(screenWidth - root.popupWidth - 15, triggerX - root.popupWidth / 2));  // Center on trigger
                ...
                if (root.triggerSection === "dock" || root.triggerSection === "center") {
                    ...
                    baseTargetY = barTop - root.popupHeight;  // 5px above dock (minimal spacing)
                    console.log(`  ControlCenter (${root.triggerSection}): Trigger-centered, calculating position`);
                } else {
                    // TASKBAR: Custom spacing above taskbar
                    baseTargetY = barTop - root.popupHeight + 45;  // Farther above bar
                    console.log(`  ControlCenter (taskbar): Trigger-centered, calculating position`);
                }
                // Ensure popup doesn't go off the top of the screen, but don't push it below the bar
                // The popup should stay above the dock/bar even when expanded
                const minY = 15;
                const maxY = barTop - root.popupHeight - 5;  // Allow slight overlap with bar area if needed
                targetY = Math.max(minY, Math.min(maxY, baseTargetY));
            }
